...
const projects = await Promise.all(
  projectIds.map(async (projectId) =>
    c.env.METADATA.get<Project>(`projects:${projectId}`, "json"),
  ),
);
...
projectRoutes.get("/:id", async (c) => {
  ...
  const project = await c.env.METADATA.get<Project>(`projects:${projectId}`, "json");
...
await Promise.all([
  c.env.METADATA.put(`projects:${projectId}`, JSON.stringify(project)),
...
await c.env.METADATA.put(`projects:${projectId}`, JSON.stringify(project));
...
await Promise.all([
  c.env.METADATA.delete(`projects:${projectId}`),
